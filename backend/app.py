...
app.config['JWT_SECRET_KEY'] = os.environ.get('SESSION_SECRET', 'dev-jwt-secret-key')
...
def get_patients():
    try:
        ...
        if current_user['user_type'] == 'doctor':
            ...
            patients = Patient.query.filter_by(caretaker_id=current_user['id']).all()
        else:
...
def get_patient(patient_id):
    try:
        ...
        vitals = PatientVitals.query.filter_by(patient_id=patient_id).order_by(PatientVitals.recorded_at.desc()).all()
